...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-3">
        <h1 class="text-primary fw-bold mb-0">🏫 Update Mark</h1>
    </div>

    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="">Update Mark</a></li>
            ...
            <form action="{{ route('marks.update',$mark->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="exam_type_id" class="form-control @error('exam_type_id') is-invalid @enderror">
                            ...
                            @foreach ($exam_types as $examtype)
                                <option value="{{ $examtype->id }}" {{ old('exam_type_id', $mark->exam_type_id ?? '') == $examtype->id ? 'selected' : '' }}>
                                    {{ $examtype->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="classroom_id" id="classroomDropdown" class="form-control @error('classroom_id') is-invalid @enderror">
                            ...
                            @foreach ($classrooms as $classroom)
                                <option value="{{ $classroom->id }}" {{ old('classroom_id', $mark->classroom_id ?? '') == $classroom->id ? 'selected' : '' }}>
                                    {{ $classroom->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" step="0.01" name="total_marks" class="form-control @error('total_marks') is-invalid @enderror" value="{{ old('total_marks', $mark->total_marks ?? '') }}">
                        @error('total_marks') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label">Written Marks</label>
                        <input type="number" step="0.01" name="obtained_marks" class="form-control @error('obtained_marks') is-invalid @enderror" value="{{ old('obtained_marks', $mark->obtained_marks ?? '') }}">
                        @error('obtained_marks') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label">Practical Marks</label>
                        <input type="number" step="0.01" name="practical_marks" class="form-control @error('practical_marks') is-invalid @enderror" value="{{ old('practical_marks', $mark->practical_marks ?? '') }}">
                        @error('practical_marks') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-12 mb-3">
                        <label class="form-label">Remarks</label>
                        <input type="text" name="remarks" class="form-control @error('remarks') is-invalid @enderror" value="{{ old('remarks', $mark->remarks ?? '') }}">
                ...
                <div class="text-end">
                    <button class="btn btn-primary px-4 mt-4">✅ Update Marks</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#classroomDropdown').on('change', function () {
            ...
            $('#student_id').html('<option disabled selected>Select Section First</option>');
        ...
        $('#sectionDropdown').on('change', function () {
            ...
            $('#student_id').html('<option disabled selected>Loading...</option>');
        ...
        const oldClassroom = "{{ old('classroom_id', $mark->classroom_id ?? '') }}";
        const oldSection = "{{ old('section_id', $mark->section_id ?? '') }}";
        const oldStudent = "{{ old('student_id', $mark->student_id ?? '') }}";

        if (oldClassroom && oldSection) {
            fetch(`/get-sections/${oldClassroom}`)
                .then(response => response.json())
                .then(sections => {
                    $('#sectionDropdown').empty().append('<option disabled selected>-Select-</option>');
                    ...
                    // Fetch students after section
                    return fetch(`/get-students/${oldClassroom}/${oldSection}`);
                })
                .then(res => res.json())
                .then(students => {
                    $('#student_id').empty().append('<option disabled selected>-Select-</option>');
                    students.forEach(student => {
                        let selected = student.id == oldStudent ? 'selected' : '';
